...
{	
	...
	$.ajax({
		url: '../php/user/getActList.php?action=getactfinished',   
		datatype:"json",
		type: "post",
		success:function(data){
	...
	function mk_acttable(product)
	{
		...
		{	
			...
			td5.align="center";td5.width="150";
			td6.align="center";td6.width="80";
			td7.align="center";td7.width="50";
			td8.align="center";td8.width="240";
			...
			else
			{
				...
				td5.innerHTML=product[i-1].actTaken+"/"+product[i-1].actLimit;
				td6.innerHTML=product[i-1].actPRID;
				td7.innerHTML=product[i-1].actRemark;
				td8.innerHTML=product[i-1].actArticle;
				...
				if(role>=0 && role<=3)	{
					$.ajax({
						...
						success:function(data){
							...
							/*if(product=="cancel")
							{
								var a_act="<a href=\"#\" onclick=\"f_DoActJoin('../php/user/deleteActivity.php?delActInfoID="+product[i-1].activityID+"','"+product[i-1].userName+" ： "+product[i-1].activityName+"');\">报名</a>";	
								td_act.innerHTML=a_act;
							}
							else if(product=="joined")
							{
								var a_act="<a href=\"#\" onclick=\"f_DoActCancel('../php/user/deleteActivity.php?delActInfoID="+product[i-1].activityID+"','"+product[i-1].userName+" ： "+product[i-1].activityName+"');\">取消报名</a>";	
								td_act.innerHTML=a_act;
							}
							else
							{
		...
		//删除活动记录
	function f_DoActJoin(url,studentInfo){
		if(confirm("是否要参加【"+studentInfo+"】活动？")){
			location.href=url;
		}
	}
	function f_DoActCancel(url,studentInfo){
		if(confirm("是否要取消参加【"+studentInfo+"】活动？")){
			location.href=url;
		}
	}
}		
