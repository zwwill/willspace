...
function clr_inpt(div_id) {
	var div=document.getElementById(div_id);
	if(div.style.borderColor!=null)
		div.style.borderColor='#0af';
}
function re_inpt(div_id) {
	//var tlt_mess=document.getElementById("tlt_mess"); 
	var div=document.getElementById(div_id);
	if(div.style.borderColor!=null)
 ...
 $(function(){
	...
	/* $.ajax({
	 ...
	 $("#sub").click(function() {
		 ...
		 var u_contact=document.getElementById("u_contact");
		 var txt_mess=document.getElementById("txt_mess");
		 if(u_name=="" || u_contact=="" || txt_mess.value=="" || txt_mess.value=="请撰写留言" ||  txt_mess.value=="留言区")
		{
			alert("有未填项！");
		}else{
			u_name=$("input#u_name").val();
			u_contact=$("input#u_contact").val(); 
			txt_mess=$("textarea#txt_mess").val();
			var dataString="txt_mess="+txt_mess+"&u_name="+u_name+"&u_contact="+u_contact;
	 		$.ajax({
				type:"post",
